...
local previewBackdropColor = {0.12, 0.12, 0.14, 1}
        ...
        if perPage < #previewList then
            ...
            for i = #previewList + 1, perPage do
                ...
                if preview == nil then
                    ...
                    preview.checkTex = preview:CreateTexture(nil, "OVERLAY")
                    preview.checkTex:SetSize(20, 20)
                    preview.checkTex:SetPoint("TOPRIGHT", -4, -4)
                    preview.checkTex:SetTexture("Interface\\RaidFrame\\ReadyCheck-Ready")
                    preview.checkTex:Hide()

                    preview.lockTex = preview:CreateTexture(nil, "OVERLAY")
                    preview.lockTex:SetSize(16, 16)
                    preview.lockTex:SetPoint("TOPLEFT", 4, -4)
                    preview.lockTex:SetTexture("Interface\\RaidFrame\\ReadyCheck-NotReady")
                    preview.lockTex:SetAlpha(0.85)
                    preview.lockTex:Hide()
                ...
                ns:QueryItem(preview.queriedItemId, function(itemId, success)
                    if itemId == preview.queriedItemId then
                        ...
                        if success then
                            ...
                            preview.isUnlocked = unlocked
                            if unlocked then
                                preview:SetAlpha(1)
                                if preview.checkTex then preview.checkTex:Show() end
                                if preview.lockTex then preview.lockTex:Hide() end
                            else
                                preview:SetAlpha(0.4)
                                if preview.checkTex then preview.checkTex:Hide() end
                                if preview.lockTex then preview.lockTex:Show() end
                            end
                        else
